...
- **Decisão de fila revertida (2026-08-02)**: a redefinição de arquitetura de 2026-08-01 havia trocado a fila de webhook de `pgmq` (Fase 1) para uma tabela Postgres simples com polling, classificando `pgmq` como dívida técnica. Essa troca nunca foi implementada; em vez disso, a quick task 260802-hvz construiu o pipeline real `sync-enqueue`/`sync-worker` diretamente sobre `pgmq` (incluindo os wrappers `pgmq_public.read`/`archive`) e provou-o ponta a ponta com dois tenants. Decisão revertida com o usuário em 2026-08-02: `pgmq` é mantido permanentemente; a migração para tabela simples não será feita. Ver `.planning/quick/260802-hvz-.../260802-hvz-SUMMARY.md`.
...
- **Sync engine (MVP)**: sem Celery/Redis — Supabase Cron (pg_cron + pg_net) dispara Edge Functions agendadas + fila `pgmq`/`pgmq_public` (`sync_work`), consumida via `pgmq_public.read`+`archive` (crash-safe, at-least-once). Decisão de trocar por tabela Postgres simples (2026-08-01) foi revertida em 2026-08-02 — `pgmq` é permanente, ver Key Decisions.
...
| Mecanismo de fila do webhook/sync: `pgmq` (extensão + wrapper `pgmq_public` + fila `sync_work`) | Implementado e verificado na Fase 1; a troca por tabela Postgres simples, confirmada em 2026-08-01, foi **revertida em 2026-08-02** após a quick task 260802-hvz construir e provar ponta a ponta (dois tenants, sem cross-contamination) o pipeline real `sync-enqueue`/`sync-worker` sobre `pgmq`/`pgmq_public.read`+`archive`. `pgmq` é a escolha definitiva; não há migração pendente. | ✓ Implementado e verificado (Fase 1 + quick-260802-hvz) |
