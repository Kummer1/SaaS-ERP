...
**Domain:** Multi-tenant SaaS backend (FastAPI) syncing a third-party ERP (Tiny/Olist) into Postgres, cost-minimized MVP hosting
**Researched:** 2026-07-27
**Confidence:** HIGH for connection/pooler mechanics and free-tier limits (verified against current official docs/discussions); MEDIUM for exact point-release version pins (verify at `pip install` time); MEDIUM for Supabase Auth Hook plan-gating (conflicting reports, see Pitfall below)

This document researches the STACK dimension for the **pivoted** decisions in `.planning/PROJECT.md` — Supabase (Postgres+Auth), no Celery/Redis, free-tier hosting. It does not re-litigate the parts of `docs/01-ARQUITETURA.md` that are unchanged (Python 3.12, FastAPI, SQLAlchemy 2.0 async, Alembic, React 18/TS/Vite — all still correct choices).
...
**If deploying the backend on Render free tier (current default):**
...
- Accept that the free web service **spins down after 15 min of inactivity** and cold-starts take 30–60s. This directly breaks two things at once: (a) inbound Tiny ERP webhooks arriving while asleep will be missed or badly delayed, and (b) the in-process APScheduler stops running entirely while the process is asleep, so your 15–30 min poll cadence silently becomes "whenever something wakes the process up."
  - **Mitigation for MVP validation only:** an external pinger (UptimeRobot free tier, or `cron-job.org`) hitting a cheap `/health` endpoint every 5–10 min keeps the process (and the scheduler) alive. This is a workaround, not a guarantee — treat webhook delivery as best-effort and lean on the **polling reconciliation loop** (already planned per `01-ARQUITETURA.md` §5) as the source of truth, exactly as the original docs already recommend for a different reason (webhook delivery isn't guaranteed even server-side).
  - **When to stop relying on this hack:** the moment there's a real paying tenant, move the backend to Render's Starter tier (~$7/mo, always-on) — reliable webhook receipt fundamentally requires a process that doesn't sleep.

**If Supabase project appears "paused":**
...
**If you later split sync workers from the API (post-MVP scale):**
...
---
*Stack research for: Multi-tenant SaaS (Tiny ERP sync) — Supabase + no-Celery + free-tier hosting pivot*
*Researched: 2026-07-27*
