...
- 2026-08-02: Root-caused and fixed the pre-existing `DATABASE_URL` password-auth failure blocking live script verification. Not a stale credential — the value in use was the Direct Connection string (port 5432) instead of the Transaction Pooler (port 6543, `aws-1-us-west-2.pooler.supabase.com`, user `postgres.<project-ref>`), and separately the DB password contains a literal `%` that must be percent-encoded (`%25`) in any connection-string URL or `postgres.js`'s URL parsing corrupts it. Confirmed live via `scripts/smoke-test-db.ts` and `scripts/verify-rls-tenant-fix.ts`, both passing against production. Also confirmed: this project has no root `.env` loading mechanism for standalone scripts — `DATABASE_URL` is read via `Deno.env.get` only (no dotenv), so it must be exported inline/in-shell for local runs; CI supplies it via a GitHub Actions secret.
...
- ~~Fix `DATABASE_URL` credential (Transaction Pooler connection string currently fails Postgres password auth)~~ — RESOLVED 2026-08-02: root cause was two-fold, not a stale credential — (1) the value in use was the Direct Connection string (port 5432, `db.<ref>.supabase.co`), not the Transaction Pooler (port 6543, `aws-1-us-west-2.pooler.supabase.com`, user `postgres.<project-ref>`); (2) the DB password contains a literal `%` character which must be percent-encoded as `%25` in any connection-string URL, or URL parsers (incl. `postgres.js`) misdecode it. Confirmed live: `scripts/smoke-test-db.ts` and `scripts/verify-rls-tenant-fix.ts` both pass against production with the corrected, properly-encoded Transaction Pooler URL. Note: this project has no root `.env` file/loading mechanism for these scripts (confirmed — `Deno.env.get` only, no dotenv import, CI passes `DATABASE_URL` via GitHub Actions secrets in the workflow's `env:` block) — `DATABASE_URL` must be exported inline or in the shell for local script runs; `supabase/functions/.env` is a separate, local-only file consumed by the local Edge Runtime, unrelated to these scripts.
- Re-run the Tiny OAuth2 connect flow (quick-260802-oam) against the **real** Tiny OAuth server once a real `client_id`/`client_secret` exists (pending a friend's Tiny account plan confirmation) — the mocked version only proves our client-side logic, not the real Tiny API contract. This is required before Phase 3's "tenant can connect their Tiny ERP account" success criterion can be considered met. **Gated 2026-08-02: do not switch off the mock flow (`tiny-mock-authorize`/`tiny-mock-token`) or point `TINY_OAUTH_AUTHORIZE_URL`/`TINY_OAUTH_TOKEN_URL` at the real Tiny endpoints until the user explicitly says to.**
- **[NEXT]** Wire `tiny-oauth-authorize`'s `tenant_id` to a real Supabase Auth session instead of a trusted query param (currently `verify_jwt=false`, documented as a known gap in the function and in `config.toml`) — needed once the dashboard "connect" button exists, and before this endpoint is production-safe. **Why this matters:** `tiny-oauth-authorize` currently reads `tenant_id` straight from the URL query string (`?tenant_id=...`) with no authentication check (`verify_jwt=false`). Anyone who can guess or enumerate a `tenants.id` UUID can call this endpoint and kick off an OAuth connect flow *as that tenant* — the resulting `tiny_credentials` row (Tiny access/refresh tokens) would be written against a tenant they don't own. Combined with RLS being keyed on `tenant_id`, this is the one remaining hole in the multi-tenant trust boundary this whole project is built to protect (see `CLAUDE.md` Multi-tenancy constraint and Phase 1's core value: "um tenant consegue conectar sua conta Tiny ERP... de forma confiável"). It's a query-param trust bug, not a missing feature — the fix is deriving `tenant_id` server-side from the caller's Supabase Auth JWT (once the dashboard "connect" button exists to originate an authenticated call), never accepting it as client-supplied input.
...
- ~~2026-08-02 (quick-260801-tef): `.env`'s `DATABASE_URL` currently fails Postgres password authentication~~ — RESOLVED 2026-08-02: see Pending Todos above. Live confirmation of the RLS tenant_id fix now closed out — both `scripts/smoke-test-db.ts` and `scripts/verify-rls-tenant-fix.ts` pass against production.
